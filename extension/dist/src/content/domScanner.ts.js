export function scanForFakeBadges() {
  const badgeKeywords = [
    "verified",
    "secure",
    "trusted",
...
export function scanForCountdownTimers() {
  ...
  if (timePatterns.test(bodyText)) {
    ...
    scripts.forEach((s) => {
      ...
      if ((code.includes("setInterval") || code.includes("countdown") || code.includes("timer")) && (code.includes("1000") || code.includes("1e3"))) {
...
export function scanContactInfo() {
  const bodyText = document.body?.innerText || "";
